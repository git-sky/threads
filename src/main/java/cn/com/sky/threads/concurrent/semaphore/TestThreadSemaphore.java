package cn.com.sky.threads.concurrent.semaphore;
...
class MyThread extends Thread {
	...
	public void run() {
		try {
			...
			// todo：也许这里可以做更复杂的业务
			System.out.println(Thread.currentThread().getName()+threadname + "成功获取了" + x + "个许可！");
		} catch (InterruptedException e) {
			e.printStackTrace();
		} finally {
			// 释放给定数目的许可，将其返回到信号量。
			pool.getSp().release(x);
			System.out.println(Thread.currentThread().getName()+threadname + "释放了" + x + "个许可！");
		}
	}
}