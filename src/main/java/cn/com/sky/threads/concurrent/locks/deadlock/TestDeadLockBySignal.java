package cn.com.sky.threads.concurrent.locks.deadlock;
...
public class TestDeadLockBySignal {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < threadNum; i++) {
			Producer p = new Producer(goods, 1, num, lock, condition);
			p.setName("P" + i);
			p.start();
		}
		...
		Consumer c = new Consumer(goods, 1, total, lock, condition);
		c.setName("C");
		c.start();
...
class Producer extends Thread {
	...
	Producer(Goods goods, int num, int runNum, Lock lock, Condition condition) {
		this.goods = goods;
		this.num = num;
		this.runNum = runNum;
		this.lock = lock;
...
class Consumer extends Thread {
	...
	Consumer(Goods goods, int num, int runNum, Lock lock, Condition condition) {
		this.goods = goods;
		this.num = num;
		this.runNum = runNum;
		this.lock = lock;
