...
//
//import cn.com.sky.threads.MyThread;
//
///**
// * <pre>
// *
// * join() 方法主要是让调用这个方法的thread完成run方法里面的东西后， 再执行join()方法后面的代码。
// * 父线程会被wait阻塞。当子线程对象t退出后，会在native方法中调用线程对象的notifyAll 方法，然后main线程被唤醒继续执行。
// *
// * 原理：
// * 其实Join方法实现是通过wait（小提示：Object 提供的方法）进行线程控制的。
// * 当main线程调用t.join时候，main线程会获得线程对象t的锁（wait 意味着拿到该对象的锁),调用该对象的wait(等待时间)，
// * 直到该对象唤醒main线程，比如退出后，线程对象t退出后，会在native方法中调用线程对象的notifyAll 方法，然后执行main线程的后续部分代码。
// *
// *
// * Java 虚拟机会在目标线程的run方法运行结束后执行该线程(对象)的 notifyAll 方法来通知所有的等待钱程。可见这里的
// * 目标线程充当了同步对象的角色，而 Java 虚拟机中 notifyAll 方法的执行线程则是通知l线程。
// *
// * </pre>
// */
