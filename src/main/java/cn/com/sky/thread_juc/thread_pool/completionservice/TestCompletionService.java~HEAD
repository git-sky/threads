 ...
 * <pre>
 *
 * 当使用ExecutorService启动了多个Callable后，每个Callable会产生一个Future，我们需要将多个Future存入一个线性表，用于之后处理数据。
 *
 * 当然，还有更复杂的情况，有5个生产者线程，每个生产者线程都会创建任务，所有任务的Future都存放到同一个线性表中。另有一个消费者线程，从线性表中取出Future进行处理。
 * CompletionService正是为此而存在，它是一个更高级的ExecutorService，它本身自带一个线程安全的线性表，无需用户额外创建。
 *
 * 它提供了2种方法从线性表中取出结果，
...
//			Future<Integer> f = completionService.poll();
            Future<Integer> f = completionService.take();//阻塞等待
            if (f == null) {
