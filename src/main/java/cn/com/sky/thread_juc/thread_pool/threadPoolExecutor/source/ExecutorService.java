...
/**
 * An {@link Executor} that provides methods to manage termination and methods that can produce a
 * {@link Future} for tracking progress of one or more asynchronous tasks.
 *
 * <p>
 * An <tt>ExecutorService</tt> can be shut down, which will cause it to reject new tasks. Two
 * different methods are provided for shutting down an <tt>ExecutorService</tt>. The
 * {@link #shutdown} method will allow previously submitted tasks to execute before terminating,
 * while the {@link #shutdownNow} method prevents waiting tasks from starting and attempts to stop
 * currently executing tasks. Upon termination, an executor has no tasks actively executing, no
 * tasks awaiting execution, and no new tasks can be submitted. An unused <tt>ExecutorService</tt>
 * should be shut down to allow reclamation of its resources.
 *
 * <p>
 * Method <tt>submit</tt> extends base method {@link Executor#execute} by creating and returning a
 * {@link Future} that can be used to cancel execution and/or wait for completion. Methods
 * <tt>invokeAny</tt> and <tt>invokeAll</tt> perform the most commonly useful forms of bulk
 * execution, executing a collection of tasks and then waiting for at least one, or all, to
 * complete. (Class {@link ExecutorCompletionService} can be used to write customized variants of
 * these methods.)
 *
 * <p>
 * The {@link Executors} class provides factory methods for the executor services provided in this
 * package.
 *
 * <h3>Usage Examples</h3>
 *
 * Here is a sketch of a network service in which threads in a thread pool service incoming
 * requests. It uses the preconfigured {@link Executors#newFixedThreadPool} factory method:
 *
 * <pre>
 ...
 * </pre>
 *
 * The following method shuts down an <tt>ExecutorService</tt> in two phases, first by calling
 * <tt>shutdown</tt> to reject incoming tasks, and then calling <tt>shutdownNow</tt>, if necessary,
 ...
 *
 * <p>
 * Memory consistency effects: Actions in a thread prior to the submission of a {@code Runnable} or
 * {@code Callable} task to an {@code ExecutorService} <a
 * href="package-summary.html#MemoryVisibility"><i>happen-before</i></a> any actions taken by that
 * task, which in turn <i>happen-before</i> the result is retrieved via {@code Future.get()}.
 *
 * @since 1.5
 * @author Doug Lea
 */
public interface ExecutorService extends Executor {
	 ...
	 *             if a security manager exists and shutting down this ExecutorService may
	 *             manipulate threads that the caller is not permitted to modify because it does not
	 *             hold {@link java.lang.RuntimePermission}<tt>("modifyThread")</tt>, or the
	 ...
	 * @throws NullPointerException
	 *             if tasks or any of its elements are <tt>null</tt>
	 * @throws RejectedExecutionException
	 ...
	 *            the time unit of the timeout argument
	 * @return a list of Futures representing the tasks, in the same sequential order as produced by
	 *         the iterator for the given task list. If the operation did not time out, each task
	 *         will have completed. If it did time out, some of these tasks will not have completed.
	 * @throws InterruptedException
	 *             if interrupted while waiting, in which case unfinished tasks are cancelled
	 * @throws NullPointerException
	 *             if tasks, any of its elements, or unit are <tt>null</tt>
	 * @throws RejectedExecutionException
	 *             if any task cannot be scheduled for execution
	 */
	 ...
	 *             if interrupted while waiting
	 * @throws NullPointerException
	 *             if tasks or any element task subject to execution is <tt>null</tt>
	 ...
	 * exceptional return, tasks that have not completed are cancelled. The results of this method
	 * are undefined if the given collection is modified while this operation is in progress.
	 *
	 * @param tasks
	 *            the collection of tasks
	 * @param timeout
	 ...
	 *             if interrupted while waiting
	 * @throws NullPointerException
	 *             if tasks, or unit, or any element task subject to execution is <tt>null</tt>
