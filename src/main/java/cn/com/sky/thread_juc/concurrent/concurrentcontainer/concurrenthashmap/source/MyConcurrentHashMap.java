...
public class MyConcurrentHashMap<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V>, Serializable {
	 ...
	 * concurrently readable hash table. To reduce footprint, all but one segments are constructed
	 * only when first needed (see ensureSegment). To maintain visibility in the presence of lazy
	 * construction, accesses to segments as well as elements of segment's table must use volatile
	 * access, which is done via Unsafe within methods segmentAt etc below. These provide the
	 * functionality of AtomicReferenceArrays but reduce the levels of indirection. Additionally,
	 * volatile-writes of table elements and entry "next" fields within locked operations use the
	 * cheaper "lazySet" forms of writes (via putOrderedObject) because these writes are always
	 * followed by lock releases that maintain sequential consistency of table updates.
	 * 
	 * Historical note: The previous version of this class relied heavily on "final" fields, which
	 * avoided some volatile reads at the expense of a large initial footprint. Some remnants of
	 * that design (including forced construction of segment 0) exist to ensure serialization
	 * compatibility.
	 */
	...
	static final class Segment<K, V> extends ReentrantLock implements Serializable {
		 ...
		 * can be read (via volatile reads of segments and tables) without locking. This requires
		 * replicating nodes when necessary during table resizing, so the old lists can be traversed
		 * by readers still using old version of table.
		 * 
		 * This class defines only mutative methods requiring locking. Except as noted, the methods
		 * of this class perform the per-segment versions of ConcurrentHashMap methods. (Other
		 * methods are integrated directly into ConcurrentHashMap methods.) These mutative methods
		 * use a form of controlled spinning on contention via methods scanAndLock and
		 * scanAndLockForPut. These intersperse tryLocks with traversals to locate nodes. The main
		 * benefit is to absorb cache misses (which are very common for hash tables) while obtaining
		 * locks so that traversal is faster once acquired. We do not actually use the found nodes
		 * since they must be re-acquired under lock anyway to ensure sequential consistency of
		 * updates (and in any case may be undetectably stale), but they will normally be much
		 * faster to re-locate. Also, scanAndLockForPut speculatively creates a fresh node to use in
		 * put if no node is found.
		 */
		...
		private void rehash(HashEntry<K, V> node) {
			/*
			 * Reclassify nodes in each list to new table. Because we are using power-of-two
			 ...
			 * referenced by any reader thread that may be in the midst of concurrently traversing
			 * table. Entry accesses use plain array indexing because they are followed by volatile
			 * table write.
			 */
